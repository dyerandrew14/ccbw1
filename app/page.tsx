...
export default function HomePage() {
  ...
  // Stop the chat - UPDATED to make an API call to cancel matchmaking
  const stopChat = useCallback(async () => {
    try {
      // Set stopping state to prevent multiple clicks
      setIsStopping(true)
      addDebugLog("Stopping chat...")
      ...
      // Make an API call to cancel matchmaking on the server
      if (isLoggedIn && isSearchingForStranger) {
        try {
          ...
          if (!response.ok) {
            console.error("Failed to cancel matchmaking:", await response.text())
            addDebugLog("Failed to cancel matchmaking on server")
          } else {
            addDebugLog("Matchmaking canceled on server")
          }
        } catch (error) {
          console.error("Error canceling matchmaking:", error)
          addDebugLog(`Error canceling matchmaking: ${error.message}`)
        }
      }
      ...
      addDebugLog(`Error in stopChat: ${error.message}`)
    } finally {
      // Reset stopping state
      setIsStopping(false)
    }
  }, [addDebugLog, addMessage, isLoggedIn, isSearchingForStranger])

  // Handle stop chat - UPDATED to prevent multiple clicks
            ...
            <div className="bg-gray-900 border-t border-gray-800 sticky bottom-0 left-0 right-0 z-10">
              <div className="grid grid-cols-2 gap-2 p-2">
                ...
                <Button
                  ...
                  disabled={isStopping}
                >
                  {isStopping ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : "Stop"}
                </Button>
